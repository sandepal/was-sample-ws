...
if "%ZIPFILE%" == "" goto DOCALL
cd "%CONFIG_ROOT%\cells\%WAS_CELL%"
jar -xf "%SAMPLES_HOME%\installableApps\%ZIPFILE%"

:DOCALL
:# Call bindings.py
call "%WAS_HOME%\bin\wsadmin" %PROFOPT% -lang jython -f "%SAMPLES_HOME%\scripts\bindings.py" attach "%WAS_POLICY%" %WAS_NODE% %WAS_CELL% %WAS_SERVER% %*
goto END
